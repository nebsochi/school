...
function Menu() {
  const { show, setShow } = useContext(ModalContext).contextValue;
  const { logOut } = useContext(AuthContext).authValue;
  const ref = useRef(null);
  const router = useRouter();
  const handleClick = (e) => {
    if (e.target.classList[0]?.startsWith("Menu_Menu__")) {
      closeMenu();
    }
  };
  ...
  const closeMenu = () => {
    ref?.current?.classList?.add(`${Styles.MenuContentRemove}`);
          ...
          <div className="card" style={{ height: "100%" }}>
            ...
            <div style={{ marginTop: "3rem" }}>
              <Link href="/">
                <a
                  className={`${Styles.MenuLink} ${
                    router.pathname === "/" ? Styles.MenuLinkActive : null
                  }`}
                >
                  <svg
              ...
              <Link href="/repayment">
                <a
                  className={`${Styles.MenuLink} ${
                    router.pathname.startsWith("/repayment")
                      ? Styles.MenuLinkActive
                      : null
                  }`}
                >
